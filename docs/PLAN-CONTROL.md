...
**שינוי-עיקרי:** 10 פריטי-תשתית:
- **P0-1:** פיצול PLAN.md ל-`context/` (מונוליט 230KB → קבצים ממוקדים)
- **P0-2:** בלוק "מצב הפרויקט" חי ב-CLAUDE.md
- **P1-1:** `docs/RISKS.md` — מרשם-סיכונים
- **P1-2:** `docs/AGENT-ACTIVITY-LOG.md` — יומן-הפעלות
- **P1-3:** תבנית-ADR עשירה (Context/Decision/Enforcement/Consequences/Rejected)
- **P1-4:** קיבוץ `docs/` לתתי-תיקיות
- **P2-1:** סעיף "Coordinate with" בקבצי-סוכן
- **P2-2:** role חדש `cross-team-auditor`
- **P2-3:** backfill ADR-001–009 לתבנית-העשירה
- **P2-4:** generator-סוכנים

**הקשר נלווה:** Part 1 (גיבוי — branch+tag+`_backup/`) ו-Part 2 (עדכון `workspace-template`) הושלמו לפני CHG זה.

### וידוא-עקביות
- (יתמלא עם השלמת הפריטים)
